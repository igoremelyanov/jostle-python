# Verify that we can Setup Google sync & storage -> 'In a hidden Google domain controlled by Jostle'.
...
class InitializeLibraryInHiddenJostleDomain(TestBaseCase, unittest.TestCase):
    def setUp(self):
        '''before Each'''
        # print 'Before Each'
        super(InitializeLibraryInHiddenJostleDomain, self).setUp()
    ...
    def tearDown(self):
        ...
        f.close()
        g.close()
        super(InitializeLibraryInHiddenJostleDomain, self).tearDown()

    def test_Can_initialize_Library_In_hidden_Google_domain_controlled_by_Jostle(self):
        global output
        ...
        '''#Lanch USQA Weaseal and make sure that we can choose option Initialize Storage -> 'In hidden Jostle Domain' '''
        ...
        output = writer("INFO:\t Jostle Weaseal Launched make sure that we can choose option Initialize Storage -> 'In Hidden Google Domain controlled by Jostle'", output)
        ...
        if jostle_Weaseal_administration_settings_page_obj.hidden_google_doamin_radio_button_click() == True:
            output = writer("VERIFY:\tWe can see Initialaze Storage Page and click 'In hidden Google Domain by Jostle' radiobutton.\tPASSED", output)
            res["P"] += 1  # add  1 to pass counter"
            assert (True)
        else:
            output = writer("VERIFY:\tCan not see Initialaze Page and cliclk 'In Hidden Google domain' radiobutton \tFAILED", output)
            self.take_screenshot('InitialazeLibrarieyOHiddenDomain_', date, now, 'FALEDclickonHiddenDomainRadioButton_000.png')
            self.failIf(True, "Test failed because Can not see Initialaze Page and cliclk 'In hidden Google domain' radiobutton ")
        ...
        if jostle_Weaseal_administration_settings_page_obj.can_see_manage_library_screen() == True:
            output = writer("VERIFY:\tWe can see Manage Library and Volume screen.\tPASSED", output)
            res["P"] += 1  # add  1 to pass counter"
            assert (True)
        else:
