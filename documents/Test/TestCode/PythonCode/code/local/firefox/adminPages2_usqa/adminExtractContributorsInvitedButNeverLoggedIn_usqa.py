...
#------------------------------------------------
class adminExtractContributorsInvitedButNeverLoggedIn(unittest.TestCase):
	...
	def test_extract_contributors(self):
		...
		self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_001.png")
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_002.png")
		...
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_003.png")
			adminGear.click()
		...
		#-------------------------
		#Find Contributors invited but never logged in
		try:
			...
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_004.png")
			viewUsageData.click()
			output = writer("INFO:\tClicked Login reports",output)
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tLogin reports is present.\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_004_"+now+"_004.png")
		#---------------------------
		#Find and select UTF-8 encoding checkbox.
		try:
			neverLoggedInForm = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"invitedContributorsForm")))
			utf8Checkbox = WebDriverWait(neverLoggedInForm, 15).until(EC.element_to_be_clickable((By.ID,"utf8Enc")))
			output = writer("VERIFY:\tUTF Encoding\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			utf8Checkbox.click()
			output = writer("INFO:\tClicked UTF Encoding",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_005.png")
		except (NoSuchElementException, TimeoutException):
		...
		try:
			neverLoggedInForm = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"invitedContributorsForm")))
			saveButton = WebDriverWait(neverLoggedInForm, 15).until(EC.element_to_be_clickable((By.ID,"saveSettings")))
			output = writer("VERIFY:\tDownload CSV Button\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsInvitedButNeverLoggedIn_"+now+"_006.png")
			saveButton.click()
		...
		if stamp:
			while True:
				try:
					...
					if "Last Name,First Name,Work Email,Work Phone,Location" in header:
