...
def if_user_exists_delete(driver, output, contributorName):
	...
	#Admin Gear Test
	try:		
		btnAdmin = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID,"QA:RichClient:btnAdmin")))		
	...
	try:		
		editDeleteContributors = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID,"QA:editDeleteContributors")))
	...
	while (flag==1):
		#Search Text Box
		try:
			searchText = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID,"searchText")))		
		...
		#Search Button
		try:
			searchButton = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID,"searchButton")))		
		...
		try:
			deleteItem = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CLASS_NAME,"delete-item")))		
		...
		if stamp:
			...
			output = writer("INFO:\tContributer wasn't deleted",output)
			#result = "fail"
			flag = 0
