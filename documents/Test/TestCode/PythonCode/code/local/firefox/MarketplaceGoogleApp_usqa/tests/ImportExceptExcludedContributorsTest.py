#  Verify that we can't Import excluded contributor "Takeuchi Len" from Google App Market to Jostle (Weaesal enterprise)
...
class ImportExceptExcludedContributorsTest(TestBaseCase, unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        '''before All tests'''
        #print 'BeforAll'
        #Proxy check for Market Palce App
        #def __init__(cls):
            #super(ImportExcludeTest, cls).setUp()
        #cls.test_Can_install_Google_Marketplace_App()
        #def __init__(cls):
        #super(ImportExcludeTest, cls).setUpClass()

        #MarketplaceAppTests.logPoint()
        #cls.logPoint('class %s' % cls.__name__)

        #InstallGoogleMarketPlaceApp()
        #cls._uno.setUp()
        #gmailLoginPageURL = TT_Constants['Gmail_Weaseal_URL']
        #cls.navigate_to_page(gmailLoginPageURL)

    @classmethod
    def tearDownClass(cls):
        ''''after All tests'''
        #print 'AfterAll'
        #super(ImportExcludeTest, cls).tearDown()

    def setUp(self):
        '''before Each'''
        #print 'Before Each'
        super(ImportExceptExcludedContributorsTest, self).setUp()
    ...
    def tearDown(self):
        ...
        super(ImportExceptExcludedContributorsTest, self).tearDown()
    ...
    #Test 1
    def test_Can_import_contributors_from_Google_except_Excluded_ones(self):
        global output
        ...
        self.assertIn("Gmail", _title) #debug
        output = writer("INFO:\tGoogle login page show up", output) #debug
        ...
        _title =self.driver.title #degbug
        print "INFO:\tI am on --", _title, "--page" #debug
        ...
        #'''#Login in Google Gmail Weaseal account'''
        #output = writer("INFO:\tLogin in Weasel Gmail", output)
        #gmailLoginPageURL = TT_Constants['Gmail_Weaseal_URL']
        #self.navigate_to_page(gmailLoginPageURL)
        #gmail_page_obj = GmailLoginPage(self.driver,
         #                               TT_Constants['Jostle_Weaseal_Username'],
        ...
        #_titlePage = gmail_title_page_obj.get_title()
        #self.assertIn(" - dwight@weaseal.com - Weaseal.com Mail", _titlePage)
        #output = writer("INFO:\tGoogle Gmail Login Launched", output)


        '''#Removing Contributor "Takeuchi Len" from Jostle (Weaseal enterprise)'''
        output = writer("INFO:\tRemoving 'Takeuchi Len' form Weaseal enterprise", output)
        ...
        try:
            ...
            #None  # abort function as pop is already closed.
        ...
        '''#Go to JostleUSQA Setup Settings and Exclude 'Len Takeuchi' '''
        ...
        #Exclude "Len" and Save
        jostleUSQA_edit_excluded_contributors_page_obj = JostleUSQAEditExcludedUsersPage(self.driver)
        jostleUSQA_edit_excluded_contributors_page_obj.add_select_option_click()
        #time.sleep(5)
        jostleUSQA_edit_excluded_contributors_page_obj.add_to_black_list_button_click()
        jostleUSQA_edit_excluded_contributors_page_obj.save_excluded_users_button_click()
        #Back to Import People Page
        self.take_screenshot('ImportExcludeTest_', date, now, 'importyourpeoplePage_004.png')
        ...
        #jostle_Weaseal_administration_settings_page_obj.search_for_contributor('Len Takeuchi')
        if jostle_Weaseal_administration_settings_page_obj.search_for_contributor('Len Takeuchi') != True:
            output = writer("VERIFY:\tExcluded contributor 'Len Takeuchi' not in Weaseal.\tPASSED", output)
            res["P"] += 1  # add  1 to pass counter"
            assert(True)
            #self.assertTrue(True, "Test passed - Len has been excluded and not presented in Jostle Weasesl")
        else:
            output = writer("VERIFY:\tExcluded contributor 'Len Takeuchi' is in Jostle Weaseal, witch is wrong.\tFAILED", output)
            #assert(False)
            #raise Exception("\"Test failed because 'Len Takeuchi' has been not excluded for some reason")
            #raiseAsserttionError
            self.take_screenshot('ImportExcludeTest_', date, now, 'FALEDsearchforExluded_000.png')
            self.failIf(True, "Test failed because 'Len Takeuchi' has been not excluded for some reason")
    ...
    #Test 2
    #def test_Can_import_All_contributors_from_Google(self):
        '''#Login in Google Gmail Weaseal account'''
        #output = writer("INFO:\tLogin in Weasel Gmail", output)
        #gmailLoginPageURL = TT_Constants['Gmail_Weaseal_URL']
        #self.navigate_to_page(gmailLoginPageURL)
