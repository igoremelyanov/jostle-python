...
# password  = "horse racing"

#------------------------------------------------
class enableGoogleUserAccess(unittest.TestCase):
	...
	def test_extract_contributors(self):
		...
		self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_001.png")
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_002.png")
		...
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_003.png")
		...
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_004.png")
			...
			verificationOption = WebDriverWait(self.driver,15).until(EC.element_to_be_clickable((By.ID,"PhoneVerificationChallenge")))
			output = writer("VERIFY:\tPhone option found\tPASS",output)
			verificationOption.click()
			output = writer("INFO:\tPhone option clicked",output)
			phoneNumber = WebDriverWait(self.driver,15).until(EC.element_to_be_clickable((By.ID,"phoneNumber")))
			output = writer("VERIFY:\tPhone input found\tPASS",output)
			phoneNumber.send_keys("7789984728")
			output = writer("INFO:\tPhone input filled",output)
			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_005WP-1.png")
			continueButton = WebDriverWait(self.driver,15).until(EC.element_to_be_clickable((By.NAME,"submitChallenge")))
			output = writer("VERIFY:\tContinue button found\tPASS",output)
			continueButton.click()
			output = writer("INFO:\tContinue button clicked",output)
			output = writer("INFO:\tpage_source"+self.driver.page_source.encode("utf-8"),output)

			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_005WP-2.png")
		except TimeoutException:
			nextButton = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"next")))
			nextButton.click()
		...
		#--------------------------
		#Check page elements
		self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_005.png")
		...
		try:
			...
			res["P"] += 1   #add  1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"enableGoogleUserAccess_"+now+"_006.png")
			adminGearIcon.click()
