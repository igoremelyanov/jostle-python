...
total = 13                                   #total number of tests
...
#------------------------------------------------
class adminExtractOverallLogins(unittest.TestCase):
	...
	def test_extract_contributors(self):
		...
		self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_001.png")
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_002.png")
		...
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_003.png")
			adminGear.click()
		...
		#-------------------------
		#Find Overall logins (weekly/monthly)
		try:
			viewUsageData = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"QA:viewUsageData")))
			output = writer("VERIFY:\tOverall logins (weekly/monthly) is present.\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_004.png")
			viewUsageData.click()
			output = writer("INFO:\tClicked Overall logins (weekly/monthly)",output)
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tOverall logins (weekly/monthly) is present.\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_004_"+now+"_004.png")
		#---------------------------
		#Check Overall Login table format
		try:
			usageDataTableItem = WebDriverWait(self.driver, 15).until(EC.visibility_of_element_located((By.XPATH,"//table[@class='usageData']/thead/tr/td")))
			if "WEEK END DATE" in usageDataTableItem.text:
				output = writer("VERIFY:\tWEEK END DATE cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tWEEK END DATE cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "PAID CONTRIBUTORS" in usageDataTableItem.text:
				output = writer("VERIFY:\tPAID CONTRIBUTORS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tPAID CONTRIBUTORS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "CREATED CONTRIBUTORS" in usageDataTableItem.text:
				output = writer("VERIFY:\tCREATED CONTRIBUTORS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tCREATED CONTRIBUTORS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "ACTIVATED CONTRIBUTORS" in usageDataTableItem.text:
				output = writer("VERIFY:\tACTIVATED CONTRIBUTORS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tACTIVATED CONTRIBUTORS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "INVITED CONTRIBUTORS" in usageDataTableItem.text:
				output = writer("VERIFY:\tINVITED CONTRIBUTORS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tINVITED CONTRIBUTORS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "UNIQUE LOGINS LAST 7 DAYS" in usageDataTableItem.text:
				output = writer("VERIFY:\tUNIQUE LOGINS LAST 7 DAYS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tUNIQUE LOGINS LAST 7 DAYS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "UNIQUE LOGINS LAST 28 DAYS" in usageDataTableItem.text:
				output = writer("VERIFY:\tUNIQUE LOGINS LAST 28 DAYS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tUNIQUE LOGINS LAST 28 DAYS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "UNIQUE LOGIN % LAST 7 DAYS" in usageDataTableItem.text:
				output = writer("VERIFY:\tUNIQUE LOGIN % LAST 7 DAYS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tUNIQUE LOGIN % LAST 7 DAYS cell is present.\tFAIL",output)

			usageDataTableItem = WebDriverWait(usageDataTableItem, 15).until(EC.visibility_of_element_located((By.XPATH,".//following-sibling::td")))
			if "UNIQUE LOGIN % LAST 28 DAYS" in usageDataTableItem.text:
				output = writer("VERIFY:\tUNIQUE LOGIN % LAST 28 DAYS cell is present.\tPASS",output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\tUNIQUE LOGIN % LAST 28 DAYS cell is present.\tFAIL",output)

			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_005.png")

		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tAll expected data about Overall Login are present.\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_005.png")
		#----------------------------
		#Download CSV Button
		try:
			downloadButton = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.NAME,"download")))
			output = writer("VERIFY:\tExtract Overall logins Button\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_006.png")
			downloadButton.click()
			output = writer("INFO:\tClicked Extract/Manage Button",output)
			stamp = True
			# time.sleep(10)
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tExtract Overall logins  Button\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractOverallLogins_"+now+"_006.png")
			stamp = False
		#--------------------------
		if stamp:
			while True:
				try:
					...
					header = g.readline()
					header = header.strip()
					output = writer("INFO:\tHeader: "+header,output)
					# Verify if header format is according to the expected.
					if(header == "Week End Date,Paid Contributors,Created Contributors,Activated Contributors,Invited Contributors,Unique Logins Last 7 Days,Unique Logins Last 28 Days,Unique Login % Last 7 Days,Unique Login % Last 28 Days"):
						output = writer("VERIFY:\tFile Download and format is OK\tPASS",output)
						res["P"] += 1   #add 1 to pass counter
					else:
						output = writer("VERIFY:\tFile Download and format is OK\tFAIL",output)
					break
				except:pass
		else:pass
		end = time.time()
