#Extract all Contributor that are not logging in data                                                                                                                                                                                                      #Extract and Manage all Contributor Data
...
#------------------------------------------------
class adminExtractContributorsNotLoggedIn(unittest.TestCase):
	...
	def test_extract_contributors(self):
		...
		self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_001.png")
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_002.png")
		...
		try:
			...
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_003.png")
			adminGear.click()
		...
		#-------------------------
		#Find Contributors not logged in (30, 60, 90 days)
		try:
			viewUsageData = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"QA:viewUsageData")))
			output = writer("VERIFY:\tLogin Reports is present.\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_004.png")
			viewUsageData.click()
			output = writer("INFO:\tClicked on Login Reports",output)
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tLogin Reports is present.\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_004_"+now+"_004.png")
		#---------------------------
		#Find report interval options and select 30 days.
		try:
			reportRange = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.XPATH,"//form[@id='contributorsNotLoggedInForm']/span")))
			if reportRange.text == " 30 days":
				output = writer("VERIFY:\t30 days report range is present.\tPASS", output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\t30 days report range is present.\tFAIL", output)
			reportRange = WebDriverWait(reportRange, 15).until(EC.element_to_be_clickable((By.XPATH,".//following-sibling::span")))
			if reportRange.text == " 60 days":
				output = writer("VERIFY:\t60 days report range is present.\tPASS", output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\t60 days report range is present.\tFAIL", output)
			reportRange = WebDriverWait(reportRange, 15).until(EC.element_to_be_clickable((By.XPATH,".//following-sibling::span")))
			if reportRange.text == " 90 days":
				output = writer("VERIFY:\t90 days report range is present.\tPASS", output)
				res["P"] += 1   #add 1 to pass counter
			else:
				output = writer("VERIFY:\t90 days report range is present.\tFAIL", output)

			output = writer("INFO:\tClicked in 30 days for report range.",output)
			WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.XPATH,"//form[@id='contributorsNotLoggedInForm']/span/input"))).click()
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_005.png")
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tAll report range options available.\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_005.png")
		#---------------------------
		#Find and select UTF-8 encoding checkbox.
		try:
			notLoggedInForm = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"contributorsNotLoggedInForm")))
			utf8Checkbox = WebDriverWait(notLoggedInForm, 15).until(EC.element_to_be_clickable((By.ID,"utf8Encoding")))
			output = writer("VERIFY:\tUTF Encoding\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			utf8Checkbox.click()
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_006.png")
		...
		try:
			notLoggedInForm = WebDriverWait(self.driver, 15).until(EC.element_to_be_clickable((By.ID,"contributorsNotLoggedInForm")))
			saveButton = WebDriverWait(notLoggedInForm, 15).until(EC.element_to_be_clickable((By.NAME,"saveSettings")))
			output = writer("VERIFY:\tDownload CSV Button\tPASS",output)
			res["P"] += 1   #add 1 to pass counter
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_007.png")
			saveButton.click()
			output = writer("INFO:\tClicked Download CSV Button",output)
			stamp = True
			# time.sleep(10)
		except (NoSuchElementException, TimeoutException):
			output = writer("VERIFY:\tDownload CSV Button\tFAIL",output)
			self.driver.save_screenshot(shot_path+os.sep+"adminExtractContributorsNotLoggedIn_"+now+"_007.png")
			stamp = False
		#-------------x-------------
		if stamp:
			while True:
				try:
					...
					if "Last Name,First Name,Work Email,Work Phone,Location,Last Login (Y/M/D)" in header:
